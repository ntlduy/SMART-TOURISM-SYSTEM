...
from backend.models import User, Shop, Comment, City, Category
import random
...
client = None
if GEMINI_API_KEY:
    app.logger.info(f"GEMINI_API_KEY loaded: {bool(GEMINI_API_KEY)}")
else:
...
if GEMINI_API_KEY and genai:
    try:
        ...
        client = True
        app.logger.info("Gemini client configured successfully from .env.")
    except Exception as e:
        app.logger.exception(f"Error configuring Gemini client: {e}")
        client = None
else:
    client = None
...
def get_gemini_response(user_message, chat_history=[]):
    ...
    try:
        ...
        app.logger.exception(f"Error calling Gemini API: {e}")
...
from backend.models import Shop, City, Category  # <--- Thêm Category vào đây
