...
import {
    ...
    ChevronRight, Image as ImageIcon, Filter, Sliders, Gift
} from 'lucide-react';
            ...
            <nav className="navbar">
                <div className="app-container nav-inner">
                    ...
                    <div className="nav-menu">
                        ...
                        <div className={`nav-item ${view === 'challenge' ? 'active' : ''}`} onClick={() => setView('challenge')}>
                            <Gift size={18} /> <span>Challenge</span>
                        </div>
                        <div className={`nav-item ${view === 'profile' ? 'active' : ''}`} onClick={() => setView('profile')}>
                            <User size={18} /> <span>Tài khoản</span>
                        </div>
                    </div>
                ...
                {view === 'profile' && (
                    <div className="fade-in">
                        ...
                        {!user ? (
                            ...
                            <ProfileView user={user} apiBase={API_BASE} />
                        )}
                    </div>
                )}
...
function ProfileView({ user, apiBase }) {
    ...
    React.useEffect(() => {
        if (!user) return;
        setLoading(true);
        fetch(`${apiBase}/api/challenge/vouchers`, { credentials: 'include' })
            .then(r => r.json())
            .then(data => {
    ...
    return (
        <div style={{ padding: 20 }}>
            ...
            <p>Điểm hiện có: <strong>{points}</strong></p>

            <div style={{ marginTop: 12 }}>
                <h4>Voucher khả dụng / đề xuất</h4>
                {loading ? (
                    <div>Đang tải...</div>
                ) : vouchers.length === 0 ? (
                    <div>Hiện chưa có voucher nào. Hoàn thành thử thách để nhận điểm.</div>
                ) : (
                    <div style={{ display: 'grid', gap: 10 }}>
                        {vouchers.map(v => (
                            <div key={v.code} style={{ border: '1px solid #eee', padding: 12, borderRadius: 8 }}>
                                <div style={{ fontWeight: 700 }}>{v.code} — {v.discount}</div>
                                <div style={{ color: '#666' }}>{v.description}</div>
                            </div>
                        ))}
                    </div>
                )}
